...
@ulapp.route('/test', methods=['GET'])
def get_forecast_for_api():
    return jsonify(compiled_forecast_data)
    ...
    if data:
        ...
        with pd.ExcelWriter(excel_filename, engine='xlsxwriter') as writer:
            for index, item in enumerate(forecast_list):
                ...
                compiled_forecast_data.update({f'Day_{day_counter}'  :  [f'{forecast_data}']})
...
if __name__ == '__main__':
    ...
    ulapp.run(debug=True, port=8080)